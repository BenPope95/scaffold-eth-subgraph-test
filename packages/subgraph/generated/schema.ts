...
export class Message extends Entity {
  ...
  set messageCount(value: BigInt | null) {
    if (!value) {
      this.unset("messageCount");
    } else {
      this.set("messageCount", Value.fromBigInt(<BigInt>value));
    }
  }
}
...
export class Messenger extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type Messenger must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("Messenger", id.toString(), this);
    }
  }

  static loadInBlock(id: string): Messenger | null {
    return changetype<Messenger | null>(store.get_in_block("Messenger", id));
  }

  static load(id: string): Messenger | null {
    return changetype<Messenger | null>(store.get("Messenger", id));
  }
